...
const openProfit = computed(() => {
  const openTrades = botStore.activeBot.openTrades;
  if (!openTrades || openTrades.length === 0) {
    return { abs: 0, ratio: 0 };
  }
  const totalStake = openTrades.reduce((a, b) => a + (b.stake_amount ?? 0), 0);
  const abs = openTrades.reduce(
  ...
  const ratio =
    totalStake > 0
      ? openTrades.reduce(
          (a, b) =>
            a + ((b.total_profit_ratio ?? b.profit_ratio ?? 0) * (b.stake_amount ?? 0)),
          0,
        ) / totalStake
      : 0;
  return { abs, ratio };
...
const closedProfit = computed(() => {
  const profit = botStore.activeBot.profit;
  if (!profit) {
    return { abs: 0, ratio: 0 };
  }
  return {
    abs: profit.profit_closed_coin ?? 0,
    ratio: profit.profit_closed_ratio ?? 0,
  };
});
    ...
    <template #default="{ gridItemProps }">
      ...
      >
        <DraggableContainer header="Multi Pane">
          <div class="mt-1 mb-2 flex flex-wrap items-center justify-center gap-x-6 gap-y-3">
            <BotControls />
            <div class="flex flex-wrap items-stretch gap-3">
              <div
                class="flex flex-col items-center gap-1 px-3 py-1.5 rounded-md bg-surface-100 dark:bg-surface-800/60 border border-surface-200 dark:border-surface-700"
              >
                <span
                  class="text-[11px] uppercase tracking-wider text-surface-500 dark:text-surface-400 font-medium"
                  >Open Profit</span
                >
                <ProfitPill
                  class="min-w-44"
                  :profit-ratio="openProfit.ratio"
                  :profit-abs="openProfit.abs"
                  :stake-currency="botStore.activeBot.stakeCurrency"
                  profit-desc="Open profit (unrealized)"
                />
              </div>
              <div
                class="flex flex-col items-center gap-1 px-3 py-1.5 rounded-md bg-surface-100 dark:bg-surface-800/60 border border-surface-200 dark:border-surface-700"
              >
                ...
                <ProfitPill
                  class="min-w-44"
                  :profit-ratio="closedProfit.ratio"
                  :profit-abs="closedProfit.abs"
                  :stake-currency="botStore.activeBot.stakeCurrency"
                  profit-desc="Closed profit (realized)"
                />
              </div>
            </div>
          </div>
